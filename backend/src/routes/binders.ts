...
router.post("/:id/cards", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (binder.userId !== req.userId) {
      res.status(403).json({ error: "Not authorized to add cards to this binder" });
      return;
    }
    ...
    const { scryfallId, pageNumber, slotPosition, ...cardData } = validation.data;
...
router.put("/:id/cards/:cardId", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
...
router.delete("/:id/cards/:cardId", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { cardId } = req.params;
...
router.patch("/:id/cards/:cardId/availability", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { cardId } = req.params;
